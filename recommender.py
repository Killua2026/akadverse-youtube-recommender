...
from fastapi import FastAPI, BackgroundTasks #FastAPI is a modern, fast (high-performance) web framework for building APIs with Python. BackgroundTasks allows us to run tasks in the background without blocking the main thread, which is useful for operations like fetching videos or building the FAISS index that might take some time.
...
def process_course_registration(student_id: str, course_topic: str):
    """
    ...
    context_vector = np.array(context_vector).astype('float32')
    distances, indices = index.search(context_vector, 3)
...
def process_career_path(student_id: str, career_name: str):
    """
...
def process_business_registration(student_id: str, business_type: str):
    """
...
def get_recommendations(student_id: str, context: str, top_k: int = 3):
    ...
    if app.state.faiss_index is None:
        return {"error": "Index is empty. Please hit /index-course first."}
